...
def chunk_document(doc: Document, settings: Settings, symbols: list[CodeSymbol] | None = None) -> list[Chunk]:
    text = doc.text
    if not text.strip():
        return []
...
def _chunk_code_document(doc: Document, settings: Settings, language: str, symbols: list[CodeSymbol]) -> list[Chunk]:
    ...
    for sym in symbols:
        ...
        for part_idx, (line_start, line_end, rel_start_char, rel_end_char) in enumerate(spans):
            ...
            end = _offset_for_line(offsets, line_end + 1) if line_end < len(offsets) else min(len(doc.text), sym.byte_end)
    ...
    for line in lines[:180]:
        ...
        if not stripped:
            if kept:
                kept.append(line)
            continue
        if stripped.startswith(("import ", "from ", "use ", "#include", "package ")) or (" from " in stripped and ("import" in stripped or "export" in stripped)):
            kept.append(line)
            continue
...
def _symbol_line_windows(sym: CodeSymbol, *, max_chars: int, overlap_lines: int) -> list[tuple[int, int, int, int]]:
    ...
    while start_idx < len(lines):
        ...
        windows.append((sym.line_start + start_idx, sym.line_start + end_idx - 1, rel_start, rel_end))
        if end_idx >= len(lines):
            break
        start_idx = max(start_idx + 1, end_idx - overlap_lines)
        char_cursor = line_offsets[start_idx]
    return windows
...
def _make_chunk(
    ...
    *,
    ...
    cid = stable_id(doc.doc_id, doc.content_hash, str(start), str(end), chunk_type, str(extra.get("part", "") if extra else ""), n=24)
    return Chunk(
        id=cid,
        doc_id=doc.doc_id,
...
def _chunk_text_document(doc: Document, settings: Settings, *, language: str = "", chunk_type: str = "text") -> list[Chunk]:
    text = doc.text
    ...
    def flush() -> None:
        ...
        if len(chunk_text) < settings.min_chunk_chars and chunks:
            ...
            line_end = _line_for_offset(offsets, cur_end)
            cid = stable_id(doc.doc_id, doc.content_hash, str(cur_start), str(cur_end), chunk_type, n=24)
            chunks.append(Chunk(
                id=cid,
                doc_id=doc.doc_id,
                root=doc.root.as_posix(),
                ...
                byte_end=cur_end,
                content_hash=doc.content_hash,
                metadata={"mtime_ns": doc.mtime_ns, "size_bytes": doc.size_bytes, "language": language, "chunk_type": chunk_type},
            ))
        cur_start = None
        cur_end = None
        cur_texts = []
