...
REGEX_SYMBOLS: dict[str, re.Pattern[str]] = {
    ...
    "javascript": re.compile(r"^([ \t]*)(?:export\s+)?(?:async\s+)?(?:function|class)\s+([A-Za-z_$][\w$]*)", re.MULTILINE),
    "typescript": re.compile(r"^([ \t]*)(?:export\s+)?(?:async\s+)?(?:function|class|interface|type|enum)\s+([A-Za-z_$][\w$]*)", re.MULTILINE),
    "tsx": re.compile(r"^([ \t]*)(?:export\s+)?(?:async\s+)?(?:function|class|interface|type|enum)\s+([A-Za-z_$][\w$]*)", re.MULTILINE),
    ...
    "java": re.compile(r"^\s*(?:public|private|protected|static|final|abstract|\s)+\s*(?:class|interface|enum)\s+([A-Za-z_][\w]*)|^\s*(?:public|private|protected|static|final|synchronized|async|\s)+[\w<>\[\], ?]+\s+([A-Za-z_][\w]*)\s*\(", re.MULTILINE),
    "cpp": re.compile(r"^\s*(?:class|struct|enum|namespace)\s+([A-Za-z_][\w]*)|^\s*[\w:<>~*&\s]+\s+([A-Za-z_~][\w:~]*)\s*\([^;]*\)\s*(?:const\s*)?(?:\{|$)", re.MULTILINE),
    "c": re.compile(r"^\s*(?:struct|enum)\s+([A-Za-z_][\w]*)|^\s*[\w_*\s]+\s+([A-Za-z_][\w]*)\s*\([^;]*\)\s*\{", re.MULTILINE),
}

IMPORT_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"^\s*(?:from\s+([\w.]+)\s+)?import\s+([\w.*, {}]+)", re.MULTILINE),
    re.compile(r"^\s*import\s+(?:type\s+)?(?:[\w*{},\s]+\s+from\s+)?['\"]([^'\"]+)['\"]", re.MULTILINE),
...
SKIP_CALL_NAMES = {
    "if", "for", "while", "switch", "catch", "return", "sizeof", "typeof", "println", "print", "assert",
    "dict", "list", "set", "tuple", "str", "int", "float", "bool", "len", "range",
}
...
def extract_references(doc: Document, symbols: list[CodeSymbol]) -> list[CodeReference]:
    ...
    for line_no, line in enumerate(lines, start=1):
        while next_symbol < len(symbols_by_start) and symbols_by_start[next_symbol].line_start <= line_no:
...
def _make_ref(doc: Document, language: str, source_symbol_id: str, target_name: str, ref_kind: str, line: int, snippet: str) -> CodeReference:
    ...
    def visit(node: Any, parents: list[str]) -> None:
        ...
        if node_type in kinds:
            ...
            if name:
                ...
                sym = CodeSymbol(
                    id=stable_id(doc.doc_id, language, qualified, str(node.start_byte), str(node.end_byte), doc.content_hash, n=28),
                    doc_id=doc.doc_id,
                    root=doc.root.as_posix(),
                    path=doc.rel_path,
                    language=language,
                ...
                symbols.append(sym)
                if kind in {"class", "interface", "trait", "struct", "enum", "impl", "module", "object", "namespace"}:
...
def _extract_symbols_regex(doc: Document, language: str) -> list[CodeSymbol]:
    ...
    for i, m in enumerate(matches):
        name = next((g.strip() for g in m.groups() if g and g.strip() and not g.isspace() and not re.fullmatch(r"async\s+def|def|class|function|interface|type|enum|struct|trait|impl|pub|export", g.strip())), "")
        if not name:
        ...
        sym = CodeSymbol(
            id=stable_id(doc.doc_id, language, name, str(start), str(end), doc.content_hash, n=28),
            doc_id=doc.doc_id,
            root=doc.root.as_posix(),
            path=doc.rel_path,
            language=language,
...
def _looks_like_import_line(line: str) -> bool:
    return line.startswith(("import ", "from ", "use ", "#include", "package ", "require(")) or " from " in line and ("import" in line or "export" in line)
...
def _import_names_from_line(line: str) -> list[str]:
    ...
    for quoted in re.findall(r"['\"]([^'\"]+)['\"]", line):
        ...
        if tail:
            names.append(tail)
    return [n for n in names if n not in {"import", "from", "as", "use", "package", "require", "export", "type"}][:64]


def _looks_like_definition_line(line: str) -> bool:
    return bool(re.match(r"^(?:async\s+)?def\s+|^(?:export\s+)?(?:async\s+)?function\s+|^func\s+|^(?:pub\s+)?(?:async\s+)?fn\s+|^(?:public|private|protected|static|final|abstract|\s)+[A-Za-z0-9_<>, ?\[\]]+\s+[A-Za-z_][\w]*\s*\(", line))
